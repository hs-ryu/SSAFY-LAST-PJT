<template>
    ...
    <a :href="'https://www.google.com/search?q=' + nowShowingMovie.movieNm" target="_blank">
      <div class="caption">
        <p class="caption_title">
          {{ nowShowingMovie.movieNm }}
        </p>
        <br v-if="nowShowingMovie.movieNm.length <= 10">
        <div class="caption_desc">
          평점 {{ (nowShowingMovie.userRating/2).toFixed(1) }} ★<br>
          개봉일 <br>
          {{ nowShowingMovie.openDt }}<br>
          누적관객 {{ nowShowingMovie.audiAcc | comma }}
          <br>
        </div>
      </div>
    </a>
  ...
  <!-- <swiper-slide>
    <img style="height: 250px;" :src="'http://image.tmdb.org/t/p/w200/' + platformMovie.poster_path" alt="" class="rounded">
    <div class="caption" @click="goToDetail">
      <p class="caption_title">
        <br>
        {{ platformMovie.title }}
      </p>
